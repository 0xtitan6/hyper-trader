...
class ConnectionHealth:
    ...
    Call `touch()` from each WS handler. Background thread checks every 15s and
    raises a single 'stale' alert when threshold is exceeded; raises a 'recovered'
    alert when traffic resumes.
    """

    def __init__(
        self, alerter: Alerter, stale_threshold_s: float = 120.0, check_interval_s: float = 15.0
    ):
