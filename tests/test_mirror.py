...
@pytest.fixture
def mt(cfg, exchange, positions, journal, alerter):
    return MirrorTrader(cfg, exchange, positions, journal, alerter)
...
def test_happy_path_live_submits(cfg, positions, journal, alerter, exchange, outcome_fill):
    cfg = _override_risk(cfg, dry_run=False)
    mt = MirrorTrader(cfg, exchange, positions, journal, alerter)
    mt.on_leader_fill("0xleader", outcome_fill)
...
def test_sell_path_subtracts_slippage(cfg, positions, journal, alerter, exchange, outcome_fill):
    cfg = _override_risk(cfg, dry_run=False)
    mt = MirrorTrader(cfg, exchange, positions, journal, alerter)
...
def test_kill_switch_blocks(mt, exchange, cfg, outcome_fill):
    Path(cfg.risk.kill_switch_file).touch()
    cfg2 = _override_risk(cfg, dry_run=False)
    mt2 = MirrorTrader(cfg2, exchange, mt.positions, mt.journal, mt.alerter)
    mt2.on_leader_fill("0xleader", outcome_fill)
    exchange.order.assert_not_called()


def test_daily_loss_blocks_and_alerts(cfg, positions, journal, exchange, outcome_fill):
    positions.realized_pnl_today.return_value = -150.0
    alerter = MagicMock()
    cfg2 = _override_risk(cfg, dry_run=False)
    mt = MirrorTrader(cfg2, exchange, positions, journal, alerter)
    mt.on_leader_fill("0xleader", outcome_fill)
    exchange.order.assert_not_called()
...
def test_exposure_cap_blocks(cfg, positions, journal, alerter, exchange, outcome_fill):
...
def test_disallowed_market_skipped(cfg, positions, journal, alerter, exchange):
...
def test_perp_allowed_when_configured(cfg, positions, journal, alerter, exchange):
...
def test_below_min_per_trade_skipped(cfg, positions, journal, alerter, exchange):
...
def test_max_per_trade_caps_size(cfg, positions, journal, alerter, exchange):
...
def test_fixed_sizing_uses_fixed_usd(cfg, positions, journal, alerter, exchange):
...
def test_order_failure_alerts_and_journals(cfg, positions, journal, exchange, outcome_fill):
    cfg2 = _override_risk(cfg, dry_run=False)
    ...
    # one for "Order submit failed", one for "Mirror pipeline exception"
    assert any("Order submit failed" in c.args[1] for c in err)


def test_journal_records_decisions(cfg, positions, exchange, alerter, outcome_fill, tmp_path):
    ...
    mt = MirrorTrader(cfg2, exchange, positions, j, alerter)
    mt.on_leader_fill("0xleader", outcome_fill)
